...
def menugen(ilist, text):
    ...
    return(select+1)
while aac != 5:
    aac = menugen(["Скретч","М.Я.У. (бета)","OSINT","О создателях","Выйти"], "DanevermasToolZ 1.0")
    try:
        os.system(cls)
    except:
        cl()
    if aac == 5:
        pass
    if aac == 3:
        print('Во первых, осинт это не слишом хорошо, во вторых, мне лень его сейчас пилить, пользуйся другими фичами приложухи [enter]')
        while not keyboard.is_pressed("enter"):
            if keyboard.is_pressed("enter"):
                pass
    if aac == 2:
        print('МЯУ (Массированный ядерный удар) будет готов... ...когда-то. Следи за обновами, чтож')
        while not keyboard.is_pressed("enter"):
            if keyboard.is_pressed("enter"):
                pass
    if aac == 4:
    ...
    if aac == 1:
        ...
        if aad == 4:
            pass
        if aad == 2:
            print('вообще, эта фича есть только В НАСТОЯЩЕЙ консоли данивемы которая есть ТОЛЬКО У МЕНЯ, так как чтоб узнать, забанен\n-ли акк, нужно знать пароль')
                ...
                if session.banned == False:
                    while aaf != 4:
                        ...
                        if aaf == 1:
                            ...
                            print("А ты почку подтвердил кракадил?:",colored(session.has_outstanding_email_confirmation,"cyan"))
